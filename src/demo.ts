...
import { encodeSignedTransactionMessage } from "./Svm/Transaction.ts"
import { buildTransactionMessage } from "./Svm/TransactionMessage.ts"
...
const program = Effect.gen(function* () {
  ...
  // Clean path (no kit pipe, no TransactionPartialSigner, no MessageWithSigners):
  //   buildTransactionMessage → encodeSignedTransactionMessage(message, [keypair])
  const transaction = yield* encodeSignedTransactionMessage(
  ...
  return { address: signerAddress, transaction }
})
